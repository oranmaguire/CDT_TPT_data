...
class DoubleDelayedGreeter(DelayedGreeter): # inherit attributes from the other class
	"""Delays double the amount of time spent, with caps option"""
	
	def __init__(self, name, caps=True):
		super().__init__(name) # super() calls the parent class' init method
		self.caps = caps
