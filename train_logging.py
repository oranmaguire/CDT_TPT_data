...
DATETIME_STR = datetime.datetime.now().strftime('%Y_%m_%dT%H_%M_%S_%f')
DB_PATH = 'mpnn_test.db'
...
def evaluate_net(net, train_dataloader, validation_dataloader, test_dataloader, criterion, args):
    ...
    if new_best_model_found:
        ...
        if args.savemodel:
            ...
            path = f'{args.dataset}{DATETIME_STR}/{args.splitmd5}/net{args.test_set[-9:-7]}'
            pathparts = path.split('/')
            if pathparts[0] not in os.listdir():
                os.makedirs('/'.join(pathparts[:-1]))


            ####todo sql route
            with open(path, 'wb+') as dest:
                nn_bytes = bz2.compress(pickle.dumps(net))
                print('n bytes!:', len(nn_bytes))
                dest.write(nn_bytes)
...
def less_log(net, train_dataloader, validation_dataloader, test_dataloader, criterion, epoch, args, timings):
    ...
    print('epoch {}, training mean {}: {}, validation mean {}: {}, testing mean {}: {}'.format(
        ...
        'cpu', timings['cpu_time'], 
        'clock', timings['monotonic']
    )
    metrics = {'epoch': epoch+1,
                'mse_train': scalars[mean_score_key]['train'],
                'mse_valid': scalars[mean_score_key]['validation'],
                'mse_test': scalars[mean_score_key]['test'],    
                'rmse_train': np.sqrt(scalars[mean_score_key]['train']),
                'rmse_valid': np.sqrt(scalars[mean_score_key]['validation']),
                'rmse_test': np.sqrt(scalars[mean_score_key]['test']), 
                'cpu_time_elapsed':  timings['cpu_time'],
                'clock_time_elapsed': timings['monotonic']
                }
    path = f'{args.dataset}{DATETIME_STR}/{args.splitmd5}/split{args.test_set[3:5]}.csv'
    header = ','.join([str(i) for i in metrics.keys()])
    data = '\n'+','.join([str(i) for i in metrics.values()])

    if epoch == 0:
        with open(path, 'w') as h:
            h.write(header)
        argx = vars(args)
        argx_header = ','.join([str(i) for i in argx.keys()])
        argx_data = ','.join([str(i) for i in argx.values()])
...
def more_log(net, train_dataloader, validation_dataloader, test_dataloader, criterion, epoch, args, timings):
    mean_score_key = 'mean {}'.format(args.score)
...
def tensorboardx_log(net, train_dataloader, validation_dataloader, test_dataloader, criterion, epoch, args, timings):
    global g
    if not g.evaluate_called:
